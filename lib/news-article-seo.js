...
import { getBusinessPostalAddress, getLocalizedUrl } from "./sitelinks-seo";
import {
  getOpeningHoursSpecification,
  getOrganizationAlternateNames,
  getSchemaInLanguage,
} from "./schema-i18n";
import { getSiteHeroUrl, getSiteLogoUrl, resolveSchemaImage } from "./schema-images";
...
/** 語系路徑：zh-TW 無前綴；en → /en；ko → /ko（與 sitelinks-seo 一致） */
...
export function getArticleUrl(siteUrl, locale, slug) {
  return getLocalizedUrl(siteUrl || SITE_URL, locale || "zh-TW", `/news/${slug}`);
}
...
function organizationNode(siteUrl, locale, siteName, siteDescription) {
  const homeUrl = getLocalizedUrl(siteUrl, locale, "/");
  const homeBase = homeUrl.replace(/\/$/, "");
  const id = `${homeBase}/#organization`;
  return {
    "@type": "Organization",
    "@id": id,
    name: siteName,
...
function blogPostingNode({
  ...
  const articleImage = resolveSchemaImage({
    candidates: [post.image, post.thumbnail],
    siteUrl,
  });
...
function webPageNode(postUrl, headline, description, post, siteUrl = SITE_URL) {
  const pageImage = resolveSchemaImage({
    candidates: [post.image, post.thumbnail],
    siteUrl,
  });
  return {
...
function productNode(product, siteUrl, locale) {
  ...
  const productImage = resolveSchemaImage({
    candidates: [product.thumbnail, product.image],
    siteUrl,
  });
...
export function buildArticleJsonLd({
  ...
  const core = [
    ...
    webPageNode(postUrl, headline, description, post, base),
    blogPostingNode({
      post,
      postUrl,
      headline,
